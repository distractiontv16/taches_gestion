...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $autoTaskStats = $this->getAutoTaskStatistics($user);
    ...
    private function getAutoTaskStatistics($user)
    {
        // Statistiques des tâches automatiques
        $totalAutoTasks = $user->tasks()->autoGenerated()->count();
        $completedAutoTasks = $user->tasks()->autoGenerated()->where('status', 'completed')->count();
        $pendingAutoTasks = $user->tasks()->autoGenerated()->where('status', '!=', 'completed')->count();

        // Tâches automatiques créées aujourd'hui
        $todayAutoTasks = $user->tasks()
            ->autoGenerated()
            ->whereDate('generation_date', Carbon::today())
            ->count();

        // Tâches automatiques créées cette semaine
        $weekAutoTasks = $user->tasks()
            ->autoGenerated()
            ->whereBetween('generation_date', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])
            ->count();

        // Routines actives
        $activeRoutines = $user->routines()->active()->count();
        $inactiveRoutines = $user->routines()->where('is_active', false)->count();

        // Taux de completion des tâches automatiques
        $autoCompletionRate = $totalAutoTasks > 0 ? round(($completedAutoTasks / $totalAutoTasks) * 100, 1) : 0;

        // Prochaines tâches automatiques (basées sur les routines actives)
        $upcomingAutoTasks = $user->tasks()
            ->autoGenerated()
            ->where('status', '!=', 'completed')
        ...
        return [
            'total_auto_tasks' => $totalAutoTasks,
            'completed_auto_tasks' => $completedAutoTasks,
            'pending_auto_tasks' => $pendingAutoTasks,
            'today_auto_tasks' => $todayAutoTasks,
            'week_auto_tasks' => $weekAutoTasks,
            'active_routines' => $activeRoutines,
            'inactive_routines' => $inactiveRoutines,
            'auto_completion_rate' => $autoCompletionRate,
            'upcoming_auto_tasks' => $upcomingAutoTasks
        ];
    }
}
